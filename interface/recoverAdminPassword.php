<?php
	...
	//message
	$message = "";
	$message_ok = $l_email_message_ok_1.$_POST['email_company'].$l_email_message_ok_2;
	...
	$Admin = Admins::checkEmailCompany($_POST);
	...
	if(!empty($Admin))
	{
		...
		$l_template_text = str_replace( 'XXXEMAIL', $_POST['email_company'], $l_template_text ); 
		...
		$sent = sentEmail($_POST['email_company'], $subject_mail,$body_mail,$from_mail);
		
		//if email is sent
		if($sent==1)
		{
			$Updated = Admins::updateAdminPassword($Admin['id'],$password_string);
			if($Updated==1)
			{
				$message = $message_ok;
				$success = 1;
